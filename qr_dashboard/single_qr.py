...
#     data = st.text_input("Enter Value", key="single_input")
...
#     if clr:
#         for k in ["qr_result","single_logged","single_input"]:
#             if k in st.session_state:
#                 del st.session_state[k]
#         st.rerun()

#     # ---- QUOTA CHECK BEFORE GENERATE ----
#     if gen:
...
#         # if quota.get("daily_remaining",0) <= 0 or quota.get("total_remaining",0) <= 0:
#         #     st.error("❌ QR limit reached. Please upgrade your plan.")
#         #     return

#         resp = requests.get(f"{FASTAPI_BASE_URL}/api/check-quota/{user_id}", timeout=10)

#         if resp.status_code != 200:
#             st.error("❌ Unable to fetch quota. FastAPI not responding.")
#             st.code(resp.text)
#             st.stop()
...
#             quota = resp.json()
#         except Exception:
#             st.error("❌ Invalid quota response from server")
#             st.code(resp.text)
#             st.stop()

...
def create_template():
    df = pd.DataFrame({"value": ["https://airobohub.com"], "status": ["Pending"]})
    df.to_excel(TEMPLATE_FILE, index=False)
...
def bulk_qr_page(user_id):
    ...
    if st.session_state.bulk_result:
        ...
        with col1:
            with open(st.session_state.bulk_result["zip"],"rb") as f:
                st.download_button("⬇ Download ZIP", f, "bulk_qr_codes.zip")
        with col2:
            with open(st.session_state.bulk_result["excel"],"rb") as f:
    ...
    if st.session_state.bulk_errors:
        ...
        st.dataframe(pd.DataFrame(st.session_state.bulk_errors))
